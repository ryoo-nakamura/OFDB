...
parser.add_argument("--category", default=10000, type=int, help="# of category")
...
if __name__ == "__main__":
    ...
    img_dir = os.path.join(
        args.save_dir, "rate" + str(args.rate) + "_category" + str(args.category)
    )
    if os.path.exists(img_dir) == False:
        os.makedirs(img_dir)

    cat_dir = os.path.join(
        args.save_dir, "csv_rate" + str(args.rate) + "_category" + str(args.category)
    )
