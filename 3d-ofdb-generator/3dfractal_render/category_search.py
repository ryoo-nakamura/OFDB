...
def conf():
    ...
    parser.add_argument(
        "--save_root",
        default="./PC-FractalDB/3DIFS_param",
        type=str,
        help="path to csv file save directory",
    )
    parser.add_argument(
        "--numof_classes", default=10000, type=int, help="PC FractalDB category number"
    )
    ...
    parser.add_argument(
        ...
        default=10000,
        type=int,
        help="PointCloud number per one 3D Fractal model",
    )
            ...
            if var_point[0] > args.variance:
                if var_point[1] > args.variance:
                    if var_point[2] > args.variance:
                        ...
                        np.savetxt(
                            "{}/{}.csv".format(args.save_root, class_str),
                            params,
                            delimiter=",",
                        )
