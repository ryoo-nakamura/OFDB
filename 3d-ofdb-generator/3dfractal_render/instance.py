...
def conf():
    ...
    parser.add_argument(
        "--load_root",
        default="./CVPR2022_fractal_param_search/var_0.1_cat10k/fractal_obj_param_list",
        type=str,
        help="load csv root",
    )
    parser.add_argument(
        "--save_root",
        default="./PC-Fractal-object/var0.10/test_sample",
        type=str,
        help="save PLY root",
    )
    parser.add_argument(
        ...
        default="point_gray",
        type=str,
        help="point_gray, point_color, patch_gray",
    )
...
if __name__ == "__main__":
    ...
    weights = np.genfromtxt(
        "./3dfractal_render/ifs_weight/weights_ins145.csv", dtype=str, delimiter=","
    )
    print(weights.shape)
    for i, csv_name in enumerate(csv_names):
        ...
        for weight in weights[0:1, :]:
            ...
            # print(padded_fractal_weight)

            params = np.genfromtxt(
                args.load_root + "/" + csv_name, dtype=str, delimiter=","
            )
            if args.draw_type == "point_gray":
                ...
                for param in params:
                    generators.set_param(
                        ...
                        float(param[11]),
                        float(param[12]),
                        weight_a=float(weight[0]),
                        weight_b=float(weight[1]),
                        weight_c=float(weight[2]),
                        weight_d=float(weight[3]),
                        weight_e=float(weight[4]),
                        weight_f=float(weight[5]),
                        weight_g=float(weight[6]),
                        weight_h=float(weight[7]),
                        weight_i=float(weight[8]),
                        weight_j=float(weight[9]),
                        weight_k=float(weight[10]),
                ...
                if arr[1] == False:
                    ...
                    # print(root +"/"+name+"_"+padded_fractal_weight+".ply")
                    open3d.io.write_point_cloud(
                        (root + "/" + name + "_" + padded_fractal_weight + ".ply"),
                        pointcloud,
                    )
            fractal_weight += 1
